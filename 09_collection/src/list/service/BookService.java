...
public class BookService {
	...
	public BookService() {
		...
		bookList.add(new BookDTO("자바 프로그래밍 입문", "박은종", 25_000));
		bookList.add(new BookDTO("선재 업고 튀어 대본집 세트", "이시은", 45_000));
		bookList.add(new BookDTO("THE MONEY BOOK", "토스", 19_800));
		
		bookList.add(new BookDTO("자바의 정석", "남궁 성", 35_000));
		bookList.add(new BookDTO("눈물의 여왕 대본집 세트", "박지은", 60_000));
		bookList.add(new BookDTO("삼국지 전권 세트", "이문열", 300_000));
	...
	/**
	 * bookList 요소의 이름 중
	 * 전달 받은 name이 포함된 이름을 모두 반환
	 * @param title
	 * @return searchList (찾은 이름이 저장된 리스트)
	 */
	...
	public String removeBook(int index) {
		...
		// * method chining : 메서드의 결과(반환값)를 이용해
		// 					  또 다른 메서드를 호출
